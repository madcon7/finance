...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const db = getDb();
  db.prepare('UPDATE declarations SET year=?, status=?, file_path=?, comment=? WHERE id=? AND user_id=?')
    .run(body.year, body.status, body.file_path || '', body.comment || '', params.id, user.userId);
