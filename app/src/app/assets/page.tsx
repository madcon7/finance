...
const MONEY_CATS = ['наличные','банковский счет'];
...
export default function AssetsPage() {
  ...
  async function loadRates() {
    ...
    try {
      const r = await apiFetch('/api/exchange-rate').then(x => x.json());
      setRates(r); setRatesLoaded(true);
    } catch {}
  }
  ...
  async function reloadFiles(id: number) {
    const data = await apiFetch(`/api/files?entity_type=asset&entity_id=${id}`).then(r => r.json());
  ...
  const isMoneyCat = MONEY_CATS.includes(form.category);
  ...
  async function openForm() {
    await loadRates();
    setForm(EMPTY); setMixedError(''); setModal('form');
  }

  async function save() {
    if (!validateMixed()) return;
    const isMoneyCategory = MONEY_CATS.includes(form.category);
    const autoName = isMoneyCategory ? (form.category === 'наличные' ? 'Наличные' : 'Банковский счет') + (form.declaration_year ? ` ${form.declaration_year}` : '') : form.name;
    ...
    await apiFetch(url, { method, body: JSON.stringify({
      ...
      original_amount: parseFloat(form.original_amount) || parseFloat(form.amount_kzt) || 0,
  ...
  async function uploadFile(assetId: number, e: React.ChangeEvent<HTMLInputElement>) {
    ...
    fd.append('file', file); fd.append('entity_type','asset'); fd.append('entity_id', String(assetId));
  ...
  function openEdit(a: any) {
    setForm({ ...a,
      ...
      original_amount: String(a.original_amount || a.amount_kzt || ''),
      currency: a.currency || 'KZT',
      exchange_rate: a.exchange_rate || 1,
  ...
  // Sell: check 1-year rule
  const sellProfit = viewAsset ? (parseFloat(sellForm.sold_amount)||0) - (viewAsset.amount_kzt||0) : 0;
  const sellHeld1Year = viewAsset?.purchase_date && sellForm.sold_date
    ? (new Date(sellForm.sold_date).getTime() - new Date(viewAsset.purchase_date).getTime()) >= 365*24*60*60*1000
    : false;
    ...
    <AppLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button onClick={openForm} className="btn-primary text-sm py-2 px-4">+ Добавить</button>
        </div>
                  ...
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {isSold ? (
                        ...
                        <div className="mt-1">
                          <p className="text-base font-bold text-blue-700">{formatKZT(a.amount_kzt)}</p>
                          {a.currency && a.currency !== 'KZT' && a.original_amount > 0 && (
                            <p className="text-xs text-slate-400">{CURRENCY_SYMBOL[a.currency]}{a.original_amount?.toLocaleString('ru')} · курс {a.exchange_rate}</p>
                          )}
                        </div>
                      )}
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-end sm:items-center justify-center">
          <div className="bg-white rounded-t-3xl sm:rounded-2xl w-full max-w-lg max-h-[95vh] overflow-y-auto">
            ...
            <div className="p-4 space-y-4 pb-6">
              ...
              {/* Simplified form for cash/bank */}
              {isMoneyCat ? (
                <>
                  <div className="bg-blue-50 rounded-2xl p-3 text-xs text-blue-600">
                    {form.category === 'наличные' ? '💵 Наличные деньги — название создастся автоматически' : '🏦 Банковский счет — название создастся автоматически'}
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    {form.currency !== 'KZT' && form.original_amount && (
                      <p className="text-xs text-slate-400 mt-1">≈ {formatKZT(parseFloat(form.amount_kzt||'0'))} · курс: 1 {form.currency} = {rates[form.currency]} ₸</p>
                    )}
                  </div>
                  <div><label className="label">Год</label><input className="input" type="number" value={form.declaration_year} onChange={e => setForm({...form, declaration_year: e.target.value})} placeholder={String(new Date().getFullYear())} /></div>
                ...
                <>
                  ...
                  {form.category === 'ценные бумаги' && (
                    <div className="bg-blue-50 rounded-2xl p-3 space-y-2">
                      ...
                      {[['company','Компания'],['ticker','Тикер'],['isin','ISIN'],['quantity','Кол-во акций'],['price_per_share','Цена за 1 акцию'],['broker','Брокер'],['broker_country','Страна брокера'],['account_number','Номер счёта']].map(([k,label]) => (
                  ...
                  {form.category === 'дивиденды' && (
                    <div className="bg-green-50 rounded-2xl p-3 space-y-2">
                      <p className="text-xs font-semibold text-green-700">💰 Данные по дивидендам</p>
                      {[['company','Компания/Эмитент'],['ticker','Тикер'],['dividend_date','Дата выплаты'],['shares_count','Кол-во акций'],['dividend_per_share','Дивиденд за акцию']].map(([k,label]) => (
                        <div key={k}><label className="label text-green-700">{label}</label><input className="input text-sm" value={form.extra_data[k]||''} onChange={e => setExtra(k, e.target.value)} /></div>
                      ))}
                    </div>
                  )}
                  {form.category === 'займ выданный' && (
                    <div className="bg-purple-50 rounded-2xl p-3 space-y-2">
                      <p className="text-xs font-semibold text-purple-700">🤝 Данные займа</p>
                      {[['borrower','Заёмщик'],['loan_date','Дата выдачи'],['due_date','Срок возврата'],['interest_rate','Ставка %']].map(([k,label]) => (
                        <div key={k}><label className="label text-purple-700">{label}</label><input className="input text-sm" value={form.extra_data[k]||''} onChange={e => setExtra(k, e.target.value)} /></div>
                      ))}
                    </div>
                  )}
                  ...
                  <div>
                    <label className="label">Стоимость {form.currency !== 'KZT' ? `в ${form.currency}` : '(₸)'} *</label>
                    <NumInput value={form.original_amount} onChange={handleOrigAmountChange} placeholder="0" />
                    {form.currency !== 'KZT' && form.original_amount && (
                      <p className="text-xs text-slate-400 mt-1">≈ {formatKZT(parseFloat(form.amount_kzt||'0'))} · курс: 1 {form.currency} = {rates[form.currency]} ₸</p>
                    )}
            ...
            <div className="p-4 border-t sticky bottom-0 bg-white flex gap-2">
              ...
              <button onClick={save} className="btn-primary flex-1" disabled={!isMoneyCat && !form.name}>Сохранить</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm">
            ...
            <div className="p-4 space-y-3">
              <div className="bg-slate-50 rounded-xl p-3 text-sm">
                ...
                {viewAsset.purchase_date && <p className="text-slate-400 text-xs">Дата покупки: {viewAsset.purchase_date}</p>}
              </div>
              ...
              {sellForm.sold_amount && (
                <div className={`rounded-xl p-3 text-sm space-y-1 ${sellProfit>0?'bg-green-50':'bg-red-50'}`}>
                  <p className={sellProfit>=0?'text-green-700':'text-red-700'}>
                    {sellProfit>=0 ? `✅ Прибыль: +${formatKZT(sellProfit)}` : `📉 Убыток: ${formatKZT(sellProfit)}`}
                  </p>
                  {sellProfit > 0 && sellHeld1Year && <p className="text-green-600 text-xs font-medium">✅ Владели более 1 года — налог не начисляется</p>}
                  {sellProfit > 0 && !sellHeld1Year && <p className="text-orange-600 text-xs">🧾 Налог ~{formatKZT(sellProfit*0.1)} (10%)</p>}
            ...
            <div className="p-4 space-y-4">
              {viewAsset.status === 'продан' ? (
                <div className="bg-slate-50 rounded-2xl p-4 space-y-2">
                  <div className="flex justify-between text-sm"><span className="text-slate-500">Куплен за:</span><span className="font-medium">{formatKZT(viewAsset.amount_kzt)}</span></div>
                  <div className="flex justify-between text-sm"><span className="text-slate-500">Продан за:</span><span className="font-bold text-lg">{formatKZT(viewAsset.sold_amount)}</span></div>
                  ...
                  {(viewAsset.profit_loss||0) > 0 && (
                    <div className="flex justify-between text-sm"><span className="text-slate-500">Налог (10%):</span><span className="font-bold text-orange-700">{formatKZT((viewAsset.profit_loss||0)*0.1)}</span></div>
                  )}
                  {taxForAsset && (
                    <div className="flex justify-between text-sm"><span className="text-slate-500">Статус налога:</span><span className={taxForAsset.status==='оплачено'?'text-green-700 font-medium':'text-red-700 font-medium'}>{taxForAsset.status}</span></div>
                  )}
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><span className="text-slate-500">Стоимость:</span><br/><span className="font-bold text-blue-700 text-base">{formatKZT(viewAsset.amount_kzt)}</span>
                    {viewAsset.currency && viewAsset.currency !== 'KZT' && <p className="text-xs text-slate-400">{CURRENCY_SYMBOL[viewAsset.currency]}{viewAsset.original_amount?.toLocaleString('ru')}</p>}
                  </div>
              ...
              {(() => {
                ...
                const labels: Record<string,string> = { company:'Компания', ticker:'Тикер', isin:'ISIN', quantity:'Кол-во', price_per_share:'Цена за акцию', broker:'Брокер', broker_country:'Страна брокера', account_number:'Счёт', coin_name:'Монета', coin_ticker:'Тикер', price:'Цена', exchange:'Биржа', wallet_address:'Кошелёк', exchange_country:'Страна биржи', borrower:'Заёмщик', loan_date:'Дата выдачи', due_date:'Срок возврата', interest_rate:'Ставка', dividend_date:'Дата выплаты', shares_count:'Акций', dividend_per_share:'Дивиденд/акция' };
                return (
